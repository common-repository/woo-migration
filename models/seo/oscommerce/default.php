...
class LeCaMgSeoOscommerceDefault
{
    ...
    public function categorySeo($cart, $category_id_desc, $convert, $category, $categoriesExt){
        ...
        if($path){
            $sql = "INSERT INTO `" . $table_name . "` (link, type, type_id) VALUES ('" . pSQL($path) . "', 'category', '" . (int)$category_id_desc . "')";
            try {
                $wpdb->query($sql);
            } catch (Exception $ex) {
    ...
    public function productSeo($cart, $product_id_desc, $convert, $product, $productsExt){
        ...
        if($data){
            foreach($data as $path){
                $sql = "INSERT INTO `" . $table_name . "` (link, type, type_id) VALUES ('" . pSQL($path) . "', 'product', '" . (int)$product_id_desc . "')";
                try {
                    $wpdb->query($sql);
                } catch (Exception $ex) {
