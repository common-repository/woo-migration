...
class LeCaMgSeoMagentoDefault
{
    ...
    public function getCategoriesExtQuery($cart, $categories){
        ...
        $ext_query = array(
            'core_url_rewrite' => "SELECT * FROM _DBPRF_core_url_rewrite WHERE product_id IS NULL AND store_id = {$default_store_src} AND category_id IN {$cat_id_con}"
        );
        return $ext_query;
    }
    ...
    public function categorySeo($cart, $category_id_desc, $convert, $category, $categoriesExt){
        ...
        $cat_urls = $cart->getListFromListByField($categoriesExt['object']['core_url_rewrite'], 'category_id', $category['entity_id']);
        if ($cat_urls) {
    ...
    public function getProductsExtQuery($cart, $products){
        ...
        $default_store_src = $notice['config']['default_lang'];
        $ext_query = array(
            'core_url_rewrite' => "SELECT * FROM _DBPRF_core_url_rewrite WHERE store_id = {$default_store_src} AND product_id IN {$pro_id_query}"
        );
        return $ext_query;
    }
    ...
    public function productSeo($cart, $product_id_desc, $convert, $product, $productsExt){
        $pro_urls = $cart->getListFromListByField($productsExt['object']['core_url_rewrite'], 'product_id', $product['entity_id']);
        if ($pro_urls) {
