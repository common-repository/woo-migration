...
class LeCaMgSeoZencartCustom{
    ...
    const SEO_ADD_CAT_PARENT = true;            // Add category parent to begining of URLs?
    ...
    public function categorySeo($cart, $category_id_desc, $convert, $category, $categoriesExt){
        ...
        $lang_map = $notice['config']['languages'];
        $cat_desc = $cart->getRowFromListByField($catDesc, 'language_id', $lang_map);
        if($cat_desc){
            if(self::SEO_ADD_CAT_PARENT){
                ...
                if($parent_cat_id){
                    ...
                    $seo_cat_desc = $cart->getRowFromListByField($seoCatDesc, 'language_id', $lang_map);
                    if($seo_cat_desc){
                        ...
                        if($parent_cat_id_lv1){
                            ...
                            $seo_cat_desc_lv1 = $cart->getRowFromListByField($seoCatDescLv1, 'language_id', $lang_map);
                            if($seo_cat_desc_lv1){
                                ...
                                if($parent_cat_id_lv2){
                                    ...
                                    $seo_cat_desc_lv2 = $cart->getRowFromListByField($seoCatDescLv2, 'language_id', $lang_map);
                                    if($seo_cat_desc_lv2){
    ...
    public function productSeo($cart, $product_id_desc, $convert, $product, $productsExt){
        ...
        $lang_map = $notice['config']['languages'];
        $pro_desc = $cart->getRowFromListByField($proDesc, 'language_id', $lang_map);
...
############################################### Extend function ################################################
    ...
    public function _toUrl($string){
        ...
        // replace certain characters with a dash
        $string = str_replace(array(
            '&',
            '&amp;',
            '?',
