...
class LeCaMgSeoWpecommerceDefault
{
    const SEO_CAT_PARENT_PATH = true;
        ...
        if(self::SEO_CAT_PARENT_PATH){
            ...
            $ext_query = array(
                'seo_categories' => "SELECT * FROM _DBPRF_term_taxonomy as tx
                      ...
                      WHERE tx.taxonomy = 'wpsc_product_category' AND tx.term_id IN {$cat_parent_ids_con}"
            );
            return $ext_query;
        }
        return false;
        ...
        if ($proCat) {
            foreach ($proCat as $pro_cat) {
                ...
                if($pro_cat['parent'] > 0){
                    ...
                    $pro_url[] = "products-page/" . $seo_path_all . $pro_cat['slug'] . '/' . $product['post_name'];
                }
            }
        }else{
            $pro_url[] = "products-page/" . $product['post_name'];
        }
        if($pro_url){
