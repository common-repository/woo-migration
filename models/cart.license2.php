    ...
    public function additionCategory($convert, $category, $categoriesExt)
{
    ...
    {       
        ...
        if (!$limit) {
            ...
            if ($limit !== 'unlimited') {
                foreach($total as $type => $count){
                    if(in_array($type, array('products', 'customers', 'orders'))){
                        $limit_count = $this->_notice['config']['limit'][$type];
                        if($limit_count === 'unlimit' || $limit_count === 'unlimited'){

                        } else {
                            $new_count = ($count < $limit_count)? $count : $limit_count;
                            $total[$type] = $new_count;
                        }
                    }
                    $this->_notice[$type]['real_total'] = $count;
                }
            }
        }
        if (LeCaMg::DEMO_MODE) {
