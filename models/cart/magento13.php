...
class LeCaMgCartMagento13 extends LeCaMgCart
{
    ...
    public function displayImport(){
        ...
        $data = $this->getConnectorData($this->getUrlConnector('query'), array(
            ...
            'query' => serialize(array(
                ...
                'products' => "SELECT COUNT(1) AS count FROM _DBPRF_catalog_product_entity WHERE type_id != 'bundle' AND entity_id NOT IN 
                    (SELECT product_id FROM _DBPRF_catalog_product_super_link UNION SELECT product_id FROM _DBPRF_catalog_product_bundle_selection UNION SELECT linked_product_id FROM _DBPRF_catalog_product_link WHERE link_type_id = 3) AND entity_id > {$this->_notice['products']['id_src']}",
                'customers' => "SELECT COUNT(1) AS count FROM _DBPRF_customer_entity WHERE entity_id > {$this->_notice['customers']['id_src']}",
                'orders' => "SELECT COUNT(1) AS count FROM _DBPRF_sales_order WHERE entity_id > {$this->_notice['orders']['id_src']}",
    ...
    protected function _getProductsMainQuery(){
        ...
        $query = "SELECT * FROM _DBPRF_catalog_product_entity WHERE type_id != 'bundle' AND entity_id NOT IN 
                    (SELECT product_id FROM _DBPRF_catalog_product_super_link UNION SELECT product_id FROM _DBPRF_catalog_product_bundle_selection UNION SELECT linked_product_id FROM _DBPRF_catalog_product_link WHERE link_type_id = 3) AND entity_id > {$id_src} ORDER BY entity_id ASC LIMIT {$limit}";
        return $query;
    }
    ...
    protected function _getProductsExtraRelQuery($products, $productsExt){
        $productIds = $this->duplicateFieldValueFromList($products['object'], 'entity_id');
        $productChildConfigIds = $this->duplicateFieldValueFromList($productsExt['object']['catalog_product_super_link'], 'product_id');
        $productChildrenGroup = $this->getListFromListByField($productsExt['object']['catalog_product_link'], 'link_type_id', '3');
        $productChildrenGroupIds = $this->duplicateFieldValueFromList($productChildrenGroup, 'linked_product_id');
        $productChildIds = array_merge($productChildConfigIds, $productChildrenGroupIds);
    ...
    public function convertProduct($product, $productsExt){
        ...
        $product_data = array(
            ...
            'post_content' => $pro_desc_std,
            'post_title' => $pro_name_std,
            'post_excerpt' => $pro_short_desc_std,
            ...
            'guid' => site_url("?product=" . sanitize_title($pro_name_std)),
            'menu_order' => 0,
            'post_type' => "product",
            'post_mime_type' => "",
            'comment_count' => 0
...
//                }
            ...
            $childrenIds = $this->duplicateFieldValueFromList($children, 'product_id');
...
//                }
        ...
        $allow = array ('select', 'text', 'textarea');
                ...
                if ($option_value = $this->getRowFromListByField($$attribute['backend_type'], 'attribute_id', $option_id_src)) {
                    ...
                    if (!$pro_attr_code) {
                        ...
                        $pro_attr_data = array(
                            'attribute_name' => $option_src_name,
                            'attribute_label' => $option_src_label,
                            'attribute_type' => $attribute['frontend_input'] == 'select' ? "select" : "text",
                            'attribute_orderby' => "menu_order",
                            'attribute_public' => 1,
                        );
                        $pro_attr_id = $this->wooAttributeTaxonomy($pro_attr_data);
                    ...
                    if ($attribute['frontend_input'] == 'select') {
                        $option_value_id_src = $option_value['value'];
                        if (!$option_value_id_src) {
                            continue;
                        }
                    } else {
                        $option_value_id_src = 0;
                    }
                    if ($pro_attr_exists) {
                        $option_src_name = $pro_attr_code;
                        $option_value_id_desc = $this->getIdDescAttributeValue($option_value_id_src);
                        if ($option_value_id_desc && $option_value_id_src) {
                            ...
                            $option_value_name = $attribute['frontend_input'] == 'select' ? $this->getRowValueFromListByField($productsExt['object']['eav_attribute_option_value'], 'option_id', $option_value_id_src, 'value') : $option_value['value'];
                            $slug = sanitize_title($option_value_name);
                            $option_value_data = array(
                                'name' => $option_value_name,
                                'slug' => $slug,
                                'term_group' => 0,
                                'taxonomy' => $option_src_name,
                                'description' => $attribute['frontend_input'] == 'select' ? $option_value_name : null,
                                'parent' => 0,
                                'count' => 0
                            );
                            $option_value_id_desc = $this->category($option_value_data);
    ...
    public function convertOrder($order, $ordersExt){
        ...
        foreach ($ordersExt['object']['eav_attribute'] as $row) {
            ...
            if ($row['entity_type_id'] == $this->_notice['extend']['order_payment']) {
                $attribute_payment[$row['attribute_code']] = $row['attribute_id'];
            }
        }
        //Eof Attribute
        //Table Attribute
        ...
        $order_status_src = $this->getRowValueFromListByField($varchar_order, 'attribute_id', $attribute_order['status'], 'value');
        ...
        $entity_payment_id = $this->getRowValueFromListByField($entity_ids, 'entity_type_id', $this->_notice['extend']['order_payment'], 'entity_id');
        $payment_info = $this->getListFromListByField($ordersExt['object']['sales_order_entity_varchar'], 'entity_id', $entity_payment_id);
        $payment_method = $this->getRowValueFromListByField($payment_info, 'attribute_id', $attribute_payment['method'], 'value');
        $meta = array(
            ...
            '_billing_country' => $this->getRowValueFromListByField($varchar_bill, 'attribute_id', $attribute_address['country_id'], 'value'),
            '_billing_first_name' => $this->getRowValueFromListByField($varchar_bill, 'attribute_id', $attribute_address['firstname'], 'value'),
            '_billing_last_name' => $this->getRowValueFromListByField($varchar_bill, 'attribute_id', $attribute_address['lastname'], 'value'),
            '_billing_company' => $this->getRowValueFromListByField($varchar_bill, 'attribute_id', $attribute_address['company'], 'value'),
            '_billing_address_1' => $this->getRowValueFromListByField($varchar_bill, 'attribute_id', $attribute_address['street'], 'value'),
            '_billing_address_2' => '',
            '_billing_city' => $this->getRowValueFromListByField($varchar_bill, 'attribute_id', $attribute_address['city'], 'value'),
            '_billing_state' => $this->getRowValueFromListByField($varchar_bill, 'attribute_id', $attribute_address['region'], 'value'),
            '_billing_postcode' => $this->getRowValueFromListByField($varchar_bill, 'attribute_id', $attribute_address['postcode'], 'value'),
            '_billing_email' => $this->getRowValueFromListByField($varchar_bill, 'attribute_id', $attribute_address['email'], 'value'),
            '_billing_phone' => $this->getRowValueFromListByField($varchar_bill, 'attribute_id', $attribute_address['telephone'], 'value'),
            '_shipping_country' => $this->getRowValueFromListByField($varchar_ship, 'attribute_id', $attribute_address['country_id'], 'value'),
            '_shipping_first_name' => $this->getRowValueFromListByField($varchar_ship, 'attribute_id', $attribute_address['firstname'], 'value'),
            '_shipping_last_name' => $this->getRowValueFromListByField($varchar_ship, 'attribute_id', $attribute_address['lastname'], 'value'),
            '_shipping_company' => $this->getRowValueFromListByField($varchar_ship, 'attribute_id', $attribute_address['company'], 'value'),
            '_shipping_address_1' => $this->getRowValueFromListByField($varchar_ship, 'attribute_id', $attribute_address['street'], 'value'),
            '_shipping_address_2' => '',
            '_shipping_city' => $this->getRowValueFromListByField($varchar_ship, 'attribute_id', $attribute_address['city'], 'value'),
            '_shipping_state' => $this->getRowValueFromListByField($varchar_ship, 'attribute_id', $attribute_address['region'], 'value'),
            '_shipping_postcode' => $this->getRowValueFromListByField($varchar_ship, 'attribute_id', $attribute_address['postcode'], 'value'),
            '_payment_method' => 'cheque',
            '_payment_method_title' => $payment_method,
    ...
    public function afterSaveOrder($order_id_desc, $convert, $order, $ordersExt){
        ...
        foreach($orderProducts as $ord_pro){
            ...
            $meta = array(
                ...
                '_line_total' => $ord_pro['row_total'] + $ord_pro['tax_amount'],
        ...
        $varchar_order = $this->getListFromListByField($ordersExt['object']['sales_order_varchar'], 'entity_id', $order['entity_id']);
        $order_item_data = array(
            'order_item_name' => $this->getRowValueFromListByField($varchar_order, 'attribute_id', $attribute_order['shipping_description'], 'value'),
        ...
        foreach($entity_status as $key => $status){
            $cmt_data = array(
                ...
                'comment_date_gmt' => $status['created_at'],
                'comment_karma' => 0,
            ...
            $history_text = $this->getListFromListByField($ordersExt['object']['sales_order_entity_text'], 'entity_id', $status['entity_id']);
            $history_int = $this->getListFromListByField($ordersExt['object']['sales_order_entity_int'], 'entity_id', $status['entity_id']);
            if($key == 0){
                $cmt_data['comment_content'] = "<b>Reference order #".$order['increment_id']."</b><br />";
                $cmt_data['comment_content'] .= "<b>Payment method: </b>".$convert['meta']['_payment_method_title']."<br />";
                $cmt_data['comment_content'] .= "<b>Shipping method: </b> ".$order_item_data['order_item_name']."<br /><br />";
                $cmt_data['comment_content'] .= $this->getRowValueFromListByField($history_text, 'attribute_id', $attribute_history['comment'], 'value');
            } else {
                $cmt_data['comment_content'] = $this->getRowValueFromListByField($history_text, 'attribute_id', $attribute_history['comment'], 'value');
            }
            ...
            if($cmt_id){
                $meta = array(
                    ...
                    'meta_value' => $this->getRowValueFromListByField($history_int, 'attribute_id', $attribute_history['is_customer_notified'], 'value') ? 1 : 0
                );
